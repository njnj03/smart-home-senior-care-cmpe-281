...
def decode_access_token(token: str) -> Optional[dict]:
    ...
    try:
        ...
        logger.error(f"JWT decode error: {e}, token preview: {token[:50] if token else 'None'}...")
        logger.error(f"SECRET_KEY length: {len(SECRET_KEY) if SECRET_KEY else 0}, ALGORITHM: {ALGORITHM}")
        return None
