    ...
    <motion.div className="fixed inset-0 bg-black/40 flex items-center justify-center p-4 z-50" initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}}>
      <motion.div className="bg-white rounded-2xl p-4 max-w-lg w-full border border-gray-200" initial={{y:20,opacity:0}} animate={{y:0,opacity:1}} exit={{y:20,opacity:0}}>
        ...
        <div className="flex gap-2 mt-4">
          {alert.status==='active'&&<button disabled={loading} className="btn" onClick={()=>act('ack')}>Acknowledge</button>}
          {(alert.status==='active'||alert.status==='acknowledged')&&<button disabled={loading} className="btn btn-primary" onClick={()=>act('res')}>Resolve</button>}
          {(alert.status==='active'||alert.status==='acknowledged')&&<button disabled={loading} className="btn" onClick={()=>act('dismiss')}>Dismiss</button>}
        </div>
      </motion.div>
    </motion.div>
  </AnimatePresence>)
